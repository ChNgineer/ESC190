...
class entry:
    def __init__ (self, word, definition, popularity):
        self.word = str(word)
...
def insert_word(N_Psi_dict, authors, word):
    '''
    ...
    if(word in N_Psi_dict):
        ...
        new_word = entry(word, definition, 0)
...
def redefine_word(N_Psi_dict, authors, word):
    '''
    ...
    authors[input("Enter new author: \n")] = None
    if(word not in N_Psi_dict):
        print("404: word not found.\n")
        return -1
    else:
...
def search_for_subset(N_Psi_dict, word, sub_range):
    if (word in N_Psi_dict) and sub_range > 0:
        ...
        print("404 word not found or inappropriate range value.\n")
        return False
...
def load_save(N_Psi_dict): 
    ...
    if(input("Are you sure you want to load a new save? Current data will be lost. (Y/N)\n").upper() == "Y"):
        ...
        for i in ['Dictionary in csv/A.csv','Dictionary in csv/B.csv','Dictionary in csv/C.csv',
                    'Dictionary in csv/D.csv','Dictionary in csv/E.csv','Dictionary in csv/F.csv',
                    'Dictionary in csv/G.csv','Dictionary in csv/H.csv','Dictionary in csv/I.csv',
                    'Dictionary in csv/J.csv','Dictionary in csv/K.csv','Dictionary in csv/L.csv',
                    'Dictionary in csv/M.csv','Dictionary in csv/N.csv','Dictionary in csv/O.csv',
                    'Dictionary in csv/P.csv','Dictionary in csv/Q.csv','Dictionary in csv/R.csv',
                    'Dictionary in csv/S.csv','Dictionary in csv/T.csv','Dictionary in csv/U.csv',
                    'Dictionary in csv/V.csv','Dictionary in csv/W.csv','Dictionary in csv/X.csv',
                    'Dictionary in csv/Y.csv','Dictionary in csv/Z.csv']:
            with open(i, newline='') as f:
                data = csv.reader(f, delimiter=' ', quotechar=' ')
                for row in data:
                    if(len(row) > 0):
                        dat = ' '.join(row).strip("\"").split("(")
                        word = dat[0].strip()
                        if((word not in N_Psi_dict)):
                            definition = (' '.join(row)).strip("\"")
                            N_Psi_dict[word] = entry(word,definition,0)
                        else:
                            definition = N_Psi_dict[word].definition
                            definition += (' '.join(row)).strip("\"")
                            N_Psi_dict[word] = entry(word,definition,0)
        return N_Psi_dict
...
def save(N_Psi_dict):
    ...
    Alpha = {"A":[],"B":[],"C":[],"D":[],"E":[],"F":[],
            ...
            "V":[],"W":[],"X":[],"Y":[],"Z":[]}
    for i in Def_list:
        if(i.definition[0].upper().isalpha() == True):
            Alpha[i.definition[0].upper()].append(i.definition)
        else:
            Alpha["A"].append(i.definition)
    for i in Alpha:
        with open('Dictionary in csv/' + i + '.csv', 'w', newline='') as f:
            data = csv.writer(f,delimiter=' ', quotechar=' ', quoting=csv.QUOTE_MINIMAL)
            for j in Alpha[i]:
                data.writerow(j.split(" "))